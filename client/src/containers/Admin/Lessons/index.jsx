import React from 'react';
import { Switch, Route, withRouter } from 'react-router-dom';
import { MDBNavLink, MDBBtn } from 'mdbreact';
import List from './List';
import Edit from './Edit';

function Lessons({ match }) {
  return (
    <React.Fragment>
      <div className='rightBordered'>
        <h3>Управление расписанием</h3>
        <br />
        <div
          className='btn-group btn-group-lg'
          role='group'
          aria-label='Games admin buttons'
        >
          <MDBNavLink to={`/admin/lessons/create`}>
            <MDBBtn className='adminBtn'>Создать занятие</MDBBtn>
          </MDBNavLink>
          <MDBNavLink to={`/admin/lessons/list`}>
            <MDBBtn className='adminBtn'>Все занятия</MDBBtn>
          </MDBNavLink>
        </div>
      </div>
      <Switch>
        <Route path={`${match.path}/`} exact component={List} />
        <Route path={`${match.path}/list`} exact component={List} />
        <Route path={`${match.path}/create`} exact component={Edit} />
        <Route
          path={`${match.path}/create/:humanId`}
          exact
          component={Edit}
        />{' '}
      </Switch>
    </React.Fragment>
  );
}

export default withRouter(Lessons);
